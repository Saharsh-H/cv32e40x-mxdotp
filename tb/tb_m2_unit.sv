...
// mxdotp_m2xfp4_fused_engine (M2XFP4 metadata-augmented MXFP4). Mirrors
// tb_fp4_unit.sv exactly - same structure, same vector-file format
// (rs3 now additionally carries the 8-bit metadata at [56:49]). NOT part of the
...
// Python golden model's own output (m2_golden.py), which was itself
...
// m2_golden.py's --emit-vectors can freely change how many vectors it
...
module tb_m2_unit;
  ...
  mxdotp_m2xfp4_fused_engine #(.X_ID_WIDTH(4), .X_RFR_WIDTH(64), .X_RFW_WIDTH(32)) dut (
    .clk_i(clk), .rst_ni(rst_n),
    .start_i(start), .ready_o(ready),
    .id_i(id_in), .rd_i(5'd10),
    .rs1_i(rs1), .rs2_i(rs2), .rs3_i(rs3),
    .mx_format_i(MX_FMT_M2XFP4),
  ...
  initial begin
    ...
    $fclose(fd);
    if (N == 0) $fatal(1, "tb_m2_unit: m2_unit_vectors.hex is empty");
    ...
    for (int i = 0; i < N; i++) begin
      ...
      if ($sscanf(line, "%h", vec[i]) != 1)
        $fatal(1, "tb_m2_unit: malformed hex line %0d: %s", i, line);
    end
    $fclose(fd);
